/*
   ...
   Derby - Class com.pivotal.gemfirexd.internal.impl.sql.compile.MethodCallNode
...
// GemStone changes END
{
	...
	*/
	...
	void throwNoMethodFound(String receiverTypeName,
									  ...
									  boolean staticMethod /* GemStoneAddition */)
		throws StandardException
	{
...
// GemStone changes BEGIN
		...
		String foundMethod = "";
		try {
		  int spaceIndex = receiverTypeName.lastIndexOf(' ');
		  String typeName = receiverTypeName;
		  if (spaceIndex != -1) {
		    typeName = receiverTypeName.substring(spaceIndex + 1);
		  }
		  Member m = classInspector.findPublicMethod(typeName,
		      methodName, null, null, null, staticMethod, false);
		  if (m != null) {
		...
		for (param = 0; param < methodParms.length; param++)
		{
			...
			if (!parameterType.equals(argumentType))
			{
				...
				if (ClassInspector.primitiveType(parameterType)) {
					...
					if (routineInfo != null) {
// GemStone changes BEGIN
					  if (routineInfo.getParameterModes()[param] != JDBC30Translation.PARAMETER_MODE_IN) {
					    continue;
					  }
					  /* (original code)
...
//		              GemStone changes begin
				if (stjvn.value.requiresTypeFromContext() && !stjvn.value.isParameterizedConstantNode())
				{
//			              GemStone changes end
					isParam[index] = true;
				}
